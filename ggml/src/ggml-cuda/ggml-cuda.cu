...
static bool ggml_cuda_graph_check_compability(ggml_cgraph * cgraph) {
    ...
    for (int i = 0; i < cgraph->n_nodes; i++) {
        ...
        // Chunked GDN allocates scratch via ggml_cuda_pool_alloc; those addresses
        // can be freed/reused between capture and replay, corrupting baked-in
        // kernel pointers. Decode is unaffected (T=1 is ineligible).
...
static int ggml_cuda_try_gdn_cache_fusion(
    ...
    // Chunked GDN prefill writes state to the gdn output tail and does not
    // scatter into the snapshot cache — skip fusion; decode and short-T fuse.

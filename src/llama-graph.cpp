...
ggml_tensor * llm_graph_context::build_attn_mha(
    ...
    if (use_flash_attn) {
        ...
        // For MLA, V is a view of K with different ne[0]. Group size must come
        // from K (which determines the WHT rotation), not V.
...
ggml_tensor * llm_graph_context::build_attn(
    ...
    // TurboQuant pre-rotate-queries: rotate Q forward to match rotated K in cache
